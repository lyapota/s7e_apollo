...
if [ -f "/tmp/aroma/ap_su_install.prop" ]; then
  INSTALL=`cat /tmp/aroma/ap_su_install.prop | grep "selected.0" | cut -f 2 -d '='`
  if [ "$INSTALL" = "2" ]; then
    # System
...
if [ -f "/tmp/aroma/ap_su_encrypt.prop" ]; then
  KEEPVERITY=`cat /tmp/aroma/ap_su_encrypt.prop | grep "selected.1" | cut -f 2 -d '='`
  ...
  KEEPFORCEENCRYPT=`cat /tmp/aroma/ap_su_encrypt.prop | grep "selected.2" | cut -f 2 -d '='`
  ...
  REMOVEENCRYPTABLE=`cat /tmp/aroma/ap_su_encrypt.prop | grep "selected.3" | cut -f 2 -d '='`
...
if [ -f "/tmp/aroma/ap_su_misc.prop" ]; then
  FRP=`cat /tmp/aroma/ap_su_misc.prop | grep "selected.1" | cut -f 2 -d '='`
  if [ "$FRP" = "2" ]; then
    # Enable
  ...
  BINDSYSTEMXBIN=`cat /tmp/aroma/ap_su_misc.prop | grep "selected.2" | cut -f 2 -d '='`
  ...
  PERMISSIVE=`cat /tmp/aroma/ap_su_misc.prop | grep "selected.3" | cut -f 2 -d '='`
